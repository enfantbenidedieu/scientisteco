...
class CONSUMPTION(BaseEstimator,TransformerMixin):
    ...
    def fit(self):
        ...
        # Optimization
        opt = minimize(objectif,x0=(0,0),method="SLSQP",constraints=const,bounds=bnds)
    ...
    def transform(self,X,new="first"):
        ...
        opt = minimize(self.objective_,x0=(0,0),method="SLSQP",constraints=new_const,bounds=bnds)
        ...
        if new in ["first-price","second-price"]:
            inter_opt =  minimize(self.objective_,x0=(0,0),method="SLSQP",constraints=inter_const,bounds=bnds)
            # Extract result
